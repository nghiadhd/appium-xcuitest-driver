...
class WebDriverAgent {
  ...
  constructor (args = {}) {
    ...
    this.webDriverAgentRunnerPath = args.webDriverAgentRunnerPath;
  ...
  async launch (sessionId) {
    ...
    if (!this.webDriverAgentRunnerPath) {
    ...
    if (this.webDriverAgentRunnerPath) {
      this.xcodebuild = await this.createXcodeTestWithoutBuildingSubProcess();
    } else {
  ...
  async createXcodeTestWithoutBuildingSubProcess () {
    let cmd = 'xcodebuild';
    let args = [
      'test-without-building',
      '-xctestrun', this.webDriverAgentRunnerPath,
  ...
  async waitForStart (startTime) {
    ...
    let startDetector = (stdout) => {
      // Don't need to wait if we run with compiled WebDriverAgent directly
      if (this.webDriverAgentRunnerPath) {
        reachedEnd = true;
      }
