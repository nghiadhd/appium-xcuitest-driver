...
const LOG_READY_REGEX = /successfully wrote manifest cache/i;
...
class WebDriverAgent {
  ...
  async createXcodeBuildSubProcess () {
    ...
    if (this.realDevice && !this.usePrebuiltWDA) {
  ...
  async startXcodebuild () {
    ...
    return await new B(async (resolve, reject) => {
      ...
      try {
        let agentUrl;
        if (this.usePrebuiltWDA) {
          agentUrl = await this.startWDA();
        } else {
          let startTime = new Date();
          await this.xcodebuild.start();
          agentUrl = await this.waitForStart(startTime);
        }
        resolve(agentUrl);
      } catch (err) {
  ...
  parseAgentUrl (stdout) {
    // WebDriverAgent exposes the server url via system logs
    //     Jul 20 13:03:57 iamPhone XCTRunner[296] <Warning>: ServerURLHere->http://10.35.4.122:8100<-ServerURLHere
    const match = AGENT_STARTED_REGEX.exec(stdout);
    if (match) {
      const agentUrl = match[1];
  ...
  startWDA () {
    return new B(async (resolve, reject) => {
      try {
        this.xcodeBuildStart = false;
        this.deviceLogs.start((stdout) => {
          if (!this.xcodeBuildStart) {
            return false;
          }

          const agentUrl = this.parseAgentUrl(stdout);
          if (agentUrl) {
            resolve(agentUrl);
            return true;
          }
        });
        // wait a few seconds to fetch old logs
        // TODO: Should find a way to clean old logs on iOS 9
        if (parseFloat(this.platformVersion) < 10.0) {
          await B.delay(5000);  
        }

        this.xcodebuild.start((stdout, stderr) => {
          let out = stdout || stderr; 
          if (LOG_READY_REGEX.test(out)) {
            log.info(out);
            this.xcodeBuildStart = true;
            return true;
          }
        });
      } catch (err) {
  ...
  async waitForStart (startTime) {
    ...
    let startDetector = (stdout) => {
      ...
      if (reachedEnd) {
        agentUrl = this.parseAgentUrl(stdout);
        if (agentUrl) {
